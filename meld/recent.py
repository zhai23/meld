...
class RecentFiles:
    ...
    mime_type: str = "application/x-meld-comparison"
    recent_path: str = os.path.join(GLib.get_user_data_dir(), "meld")
    recent_suffix: str = ".meldcmp"

    # Recent data
    app_name: str = "Meld"

    def __init__(self) -> None:
        self.recent_manager: Gtk.RecentManager = Gtk.RecentManager.get_default()
        self.recent_filter: Gtk.RecentFilter = Gtk.RecentFilter()
        self.recent_filter.add_mime_type(self.mime_type)
        self._stored_comparisons: Dict[Tuple[RecentType, Tuple[str, ...]], str] = {}
        self.app_exec: str = os.path.abspath(sys.argv[0])
    ...
    def add(self, tab: Any, flags: Optional[Any] = None) -> None:
        ...
        uris: List[str] = [f.get_uri() for f in gfiles]
        if not all(uris):
            return

        names: List[str] = [f.get_parse_name() for f in gfiles]

        # If a (type, uris) comparison is already registered, then re-add
        # the corresponding comparison file
        comparison_key: Tuple[RecentType, Tuple[str, ...]] = (recent_type, tuple(uris))
        if comparison_key in self._stored_comparisons:
            gfile: Gio.File = Gio.File.new_for_uri(
                self._stored_comparisons[comparison_key])
        else:
            recent_path: str = self._write_recent_file(recent_type, uris)
            gfile = Gio.File.new_for_path(recent_path)

        if len(uris) > 1:
            display_name: str = " : ".join(meld.misc.shorten_names(*names))
        else:
            display_path: str = names[0]
        ...
        recent_metadata: Gtk.RecentData = Gtk.RecentData()
    ...
    def read(self, uri: str) -> Tuple[RecentType, List[Gio.File]]:
        ...
        comp_gfile: Gio.File = Gio.File.new_for_uri(uri)
        comp_path: Optional[str] = comp_gfile.get_path()
        ...
        uris: List[str] = config.get("Comparison", "uris").split(";")
        gfiles: List[Gio.File] = [Gio.File.new_for_uri(u) for u in uris]

        return recent_type, gfiles

    def _write_recent_file(self, recent_type: RecentType, uris: List[str]) -> str:
    ...
    def _clean_recent_files(self) -> None:
        # Remove from RecentManager any comparisons with no existing file
        meld_items: List[Gtk.RecentInfo] = self._filter_items(self.recent_filter,
        ...
        item_uris: List[str] = [item.get_uri() for item in meld_items]
        item_paths: List[Optional[str]] = [
            Gio.File.new_for_uri(uri).get_path() for uri in item_uris]
        stored: List[str] = [p for p in os.listdir(self.recent_path)
                  if p.endswith(self.recent_suffix)]
        for path in stored:
            file_path: str = os.path.abspath(os.path.join(self.recent_path, path))
    ...
    def _update_recent_files(self, *args: Any) -> None:
        meld_items: List[Gtk.RecentInfo] = self._filter_items(self.recent_filter,
                                        self.recent_manager.get_items())
        item_uris: List[str] = [item.get_uri() for item in meld_items if item.exists()]
        ...
        for item_uri in item_uris:
            ...
            gfile_uris: Tuple[str, ...] = tuple(gfile.get_uri() for gfile in gfiles)
            self._stored_comparisons[recent_type, gfile_uris] = item_uri

    def _filter_items(self, recent_filter: Gtk.RecentFilter, items: List[Gtk.RecentInfo]) -> List[Gtk.RecentInfo]:
        getters: Dict[Gtk.RecentFilterFlags, str] = {Gtk.RecentFilterFlags.URI: "uri",
        ...
        needed: Gtk.RecentFilterFlags = recent_filter.get_needed()
        attrs: List[str] = [v for k, v in getters.items() if needed & k]

        filtered_items: List[Gtk.RecentInfo] = []
        for i in items:
            filter_data: Dict[str, Any] = {}
            for attr in attrs:
                filter_data[attr] = getattr(i, "get_" + attr)()
            filter_info: Gtk.RecentFilterInfo = Gtk.RecentFilterInfo()
    ...
    def __str__(self) -> str:
        items: List[Gtk.RecentInfo] = self.recent_manager.get_items()
...
recent_comparisons: RecentFiles = RecentFiles()
