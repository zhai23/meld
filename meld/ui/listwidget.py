...
class EditableListWidget:
    ...
    def setup_sensitivity_handling(self) -> None:
    ...
    def _update_sensitivity(self, *args: Any) -> None:
    ...
    def _get_selected(self) -> Tuple[Gtk.TreeModel, Optional[Gtk.TreeIter], Optional[int]]:
    ...
    def move_up_selected_entry(self) -> None:
        model, it, path = self._get_selected()
        model.swap(it, model.get_iter(path - 1))

    def move_down_selected_entry(self) -> None:
        model, it, path = self._get_selected()
        model.swap(it, model.get_iter(path + 1))
